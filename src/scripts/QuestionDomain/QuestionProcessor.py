...
class QuestionProcessor:
    def __init__(self):
        ...
        self.elements = {}
        with open('csv/TAG_ELEMENT.csv') as csv_file:
                    csv_reader = csv.reader(csv_file, delimiter=',')
                    for row in csv_reader:
                        elements[row[0]] = row[1:-1]

        self.ordinal = {}
        with open('csv/TAG_ORDINAL.csv') as csv_file:
                    csv_reader = csv.reader(csv_file, delimiter=',')
                    for row in csv_reader:
                        ordinal[row[0]] = int(row[1])

    def searchprocess(self,tree,question=''):
        path = self.generatePath(question)
        if tree.pathExist(path):
            node = tree.goto(path)
            closest = self.getClosestMatch(node,question)
    ...
    def constructPath(self,sentence=[]):
        ...
        for word in sentence:
            for key, lst in elements.items():
                if word in lst:
                    pathATR[key] = word 
                    sentence.remove(word)
                    break
        sortedPath = {k:v for k, v in sorted(pathATR.items(), key=lambda item: getOrdinalByKey(item[0]) )}
    ...
    def removeSynonym(self,input=[]):
        ...
        while len(input) > 0:
            word = input.pop(0)
            synonymDictionary = getSynonymList(word)
    ...
    def getSynonymList(self,word):
        synonyms = most_similar(self.nlp.vocab[word]) 
    ...
    def getClosestMatch(questionNode,sentence):
        return difflib.get_close_matches(sentence,questionNode.questions)[0]
